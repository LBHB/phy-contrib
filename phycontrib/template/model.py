...
def load_metadata(filename):
    ...
    with open(filename, 'r') as f:
        ...
        # Skip the header.
        for row in reader:
            should_be_cluster_id= row[0]
            field_names = row[1:]
            if should_be_cluster_id != 'cluster_id':
                #this is an unexpected file format, return nothing    
                return dic
            dic = [{} for _ in range(len(field_names))]
            break
        for row in reader:
            cluster = row[0]
            values = row[1:]
            ...
            for i in range(len(field_names)):
                dic[i][cluster] = values[i]
    return field_names, dic
    ...
    def _load_metadata(self):
        ...
        for filename in files:
            ...
            for i in range(len(field_name)):
                metadata[field_name[i]] = values[i]
        return metadata
